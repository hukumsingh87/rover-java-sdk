...
import okhttp3.*;
import org.rover.sdk.requests.ResponseHandler;
 ...
 * <a href="https://www.rover.network/developers/guides/concepts/federation.html" target="_blank">federation server</a>
 * instance and exposes an interface for requests to that instance.
 *
 * For resolving a rover address without knowing which federation server
...
public class FederationServer {
   ...
   * It tries to find a federation server URL in rover.toml file.
   * @see <a href="https://www.rover.network/developers/guides/concepts/rover-toml.html" target="_blank">Rover.toml docs</a>
  ...
  public static FederationServer createForDomain(String domain) {
    ...
    uriBuilder.append("/.well-known/rover.toml");
    HttpUrl roverTomlUri = HttpUrl.parse(uriBuilder.toString());
    OkHttpClient httpClient = FederationServer.createHttpClient();

    Request request = new Request.Builder().get().url(roverTomlUri).build();
    Response response = null;
    try {
      response = httpClient.newCall(request).execute();

      if (response.code() >= 300) {
        throw new RoverTomlNotFoundInvalidException();
      }

      Toml roverToml = new Toml().read(response.body().string());

      String federationServer = roverToml.getString("FEDERATION_SERVER");
  ...
  /**
   * Resolves a rover address using a given federation server.
   * @param address Rover addres, like <code>bob*rover.network</code>

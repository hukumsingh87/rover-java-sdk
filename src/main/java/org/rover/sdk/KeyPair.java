...
public class KeyPair {
  ...
  /**
   * Creates a new Rover KeyPair from a strkey encoded Rover secret seed.
   * @param seed Char array containing strkey encoded Rover secret seed.
   * @return {@link KeyPair}
   */
  public static KeyPair fromSecretSeed(char[] seed) {
    byte[] decoded = StrKey.decodeRoverSecretSeed(seed);
  ...
  /**
   * <strong>Insecure</strong> Creates a new Rover KeyPair from a strkey encoded Rover secret seed.
   ...
   * @param seed The strkey encoded Rover secret seed.
   * @return {@link KeyPair}
   */
  public static KeyPair fromSecretSeed(String seed) {
    char[] charSeed = seed.toCharArray();
    byte[] decoded = StrKey.decodeRoverSecretSeed(charSeed);
    KeyPair keypair = fromSecretSeed(decoded);
  ...
  /**
   * Creates a new Rover keypair from a raw 32 byte secret seed.
  ...
  /**
   * Creates a new Rover KeyPair from a strkey encoded Rover account ID.
   * @param accountId The strkey encoded Rover account ID.
   * @return {@link KeyPair}
   */
  public static KeyPair fromAccountId(String accountId) {
    byte[] decoded = StrKey.decodeRoverAccountId(accountId);
    return fromPublicKey(decoded);
  }

  /**
   * Creates a new Rover keypair from a 32 byte address.
  ...
  /**
   * Generates a random Rover keypair.
   * @return a random Rover keypair.
   */
  ...
  public String getAccountId() {
    return StrKey.encodeRoverAccountId(mPublicKey.getAbyte());
  }
  ...
  public char[] getSecretSeed() {
    return StrKey.encodeRoverSecretSeed(mPrivateKey.getSeed());
  }

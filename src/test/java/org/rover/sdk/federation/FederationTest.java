...
public class FederationTest extends TestCase {
  ...
  public void testResolveSuccess() throws IOException {
    ...
    String domain = String.format("%s:%d", baseUrl.host(), baseUrl.port());
      ...
      String successResponse =
              "{\"rover_address\":\"bob*"+domain+"\",\"account_id\":\"GCW667JUHCOP5Y7KY6KGDHNPHFM4CS3FCBQ7QWDUALXTX3PGXLSOEALY\"}";
    ...
    assertEquals(response.getRoverAddress(), "bob*"+domain);
  ...
  public void testMalformedAddress() {
    try {
      FederationResponse response = Federation.resolve("bob*rover.network*test");
